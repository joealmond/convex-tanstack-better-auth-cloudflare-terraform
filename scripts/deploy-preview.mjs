...
if (!isConvexCloudPreview(env)) {
  throw new Error('Preview deploy requires a Convex cloud development deployment, not a local backend.')
}

run('node', ['scripts/deploy-preflight.mjs', '--environment', 'preview'], { stdio: 'pipe', env: { ...process.env, APP_URL: appUrl, REQUIRE_LOCAL_SECRET: 'true' } })
...
if (!configuredDomain && !workersUrl) {
  throw new Error('Wrangler did not report a workers.dev URL. Configure a Custom Domain or retry the deployment.')
}
...
run('node', ['scripts/write-release-record.mjs'], {
  stdio: 'pipe',
  env: { ...process.env, DEPLOY_ENVIRONMENT: 'preview', APP_URL: appUrl, CLOUDFLARE_WORKER_NAME: workerName, WORKER_VERSION_ID: workerVersion, VITE_CONVEX_URL: env.VITE_CONVEX_URL, CONVEX_DEPLOYMENT: env.CONVEX_DEPLOYMENT },
})
