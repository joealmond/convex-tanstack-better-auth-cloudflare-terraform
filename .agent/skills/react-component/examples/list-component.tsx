...
export function ItemList({ onItemClick, statusFilter, limit }: ItemListProps) {
  ...
  return (
    <ul className="space-y-3" role="list">
      {items.map((item) => (
        <ItemCard key={item._id} item={item} onClick={() => onItemClick?.(item._id)} />
      ))}
    </ul>
  )
}
...
function StatusBadge({ status }: StatusBadgeProps) {
  ...
  return (
    <span className={`ml-2 px-2 py-1 text-xs font-medium rounded ${styles[status]}`}>{status}</span>
  )
}
...
function ItemListEmpty({ statusFilter }: ItemListEmptyProps) {
  const message = statusFilter ? `No ${statusFilter} items` : 'No items yet'
