...
export async function requireOrganizationRole(ctx: MutationCtx, organizationId: Id<'organizations'>, userId: string, minimum: Role = 'member') {
  const membership = await ctx.db.query('organizationMembers').withIndex('by_organization_user', (q) => q.eq('organizationId', organizationId).eq('userId', userId)).unique()
  if (!membership || rank[membership.role] < rank[minimum]) throw new ConvexError('Organization access denied')
  return membership
}

async function audit(ctx: MutationCtx, organizationId: Id<'organizations'>, actorId: string, event: string) {
  await ctx.db.insert('organizationAuditEvents', { organizationId, actorId, event, createdAt: Date.now() })
}
...
export const create = authMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.insert('organizationMembers', { organizationId, userId: ctx.userId, role: 'owner' })
    await audit(ctx, organizationId, ctx.userId, 'organization.created')
    return organizationId
  },
})

export const mine = authQuery({ args: {}, handler: (ctx) => ctx.db.query('organizationMembers').withIndex('by_user', (q) => q.eq('userId', ctx.userId)).collect() })
...
export const invite = authMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const values = { ...args, email, invitedBy: ctx.userId, expiresAt: Date.now() + 7 * 24 * 60 * 60 * 1000 }
...
export const acceptInvitation = authMutation({
  ...
  handler: async (ctx, { organizationId }) => {
    ...
    const invitation = await ctx.db.query('organizationInvitations').withIndex('by_organization_email', (q) => q.eq('organizationId', organizationId).eq('email', ctx.user.email.toLowerCase())).unique()
    if (!invitation || invitation.expiresAt < Date.now()) throw new ConvexError('Invitation is invalid or expired')
    if (!await ctx.db.query('organizationMembers').withIndex('by_organization_user', (q) => q.eq('organizationId', organizationId).eq('userId', ctx.userId)).unique()) await ctx.db.insert('organizationMembers', { organizationId, userId: ctx.userId, role: invitation.role })
...
export const setEntitlement = authMutation({
  ...
  handler: async (ctx, args) => {
    await requireOrganizationRole(ctx, args.organizationId, ctx.userId, 'admin')
    const existing = await ctx.db.query('organizationEntitlements').withIndex('by_organization_key', (q) => q.eq('organizationId', args.organizationId).eq('key', args.key)).unique()
...
export const hasEntitlement = authQuery({
  ...
  handler: async (ctx, args) => {
    ...
    return (await ctx.db.query('organizationEntitlements').withIndex('by_organization_key', (q) => q.eq('organizationId', args.organizationId).eq('key', args.key)).unique())?.enabled ?? false
  },
})
