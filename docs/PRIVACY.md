...
When the billing example is selected, active subscriptions or pending Checkout sessions block account deletion until they are cancelled or expire. After a billable account is deleted, the backend retains a minimal user/provider-ID tombstone to reject delayed Stripe webhooks. Set and document a retention policy for that record before launch.
